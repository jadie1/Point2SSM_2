...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.cls_module = Classification_Module(self.num_output*3, self.num_classes)
    ...
    def forward(self, x, gt, labels, epoch=0):
        ...
        # Classification
        class_prob = self.cls_module(full_pred[:x.shape[0],])
        ...
        cls_loss = F.nll_loss(class_prob, labels).mean()

        if epoch < self.consistency_start_epoch:
            loss = base_loss + self.cls_weight*cls_loss
        else:
...
class Classification_Module(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        self.fc2 = nn.Linear(self.input_dim//2, self.input_dim//4)
        self.fc3 = nn.Linear(self.input_dim//4, self.num_classes)

        self.bn1 = nn.BatchNorm1d(self.input_dim//2)
        self.bn2 = nn.BatchNorm1d(self.input_dim//4)      
