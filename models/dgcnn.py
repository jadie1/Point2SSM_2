...
class DGCNN_encoder(nn.Module):
    def __init__(self, latent_dim, k=27, depth=4, bb_size=24, extra_features=0):
    ...
    def forward(self, x, return_neighs=False):
        ...
        edge_index = [
            torch_cluster.knn(x[i,:,:3], x[i,:,:3], self.num_neighs,)
...
class DGCNN_encoder_all_neighs(nn.Module):
    def __init__(self, latent_dim, k=27, extra_features=0):
        super(DGCNN_encoder_all_neighs, self).__init__()
        self.num_neighs = k
        self.latent_dim = latent_dim
        self.input_features = (3+extra_features) * 2
        self.only_true_neighs = True
    ...
    def forward_per_point(self, x, start_neighs=None):
        ...
        if(start_neighs is None):
            start_neighs = torch_cluster.knn(x,k=self.num_neighs)
        x = get_graph_feature(x, k=self.num_points, idx=start_neighs, only_intrinsic=False)#only_intrinsic=self.hparams.only_intrinsic)
        other = x[:,:3,:,:]
    ...
    def forward(self, x, return_neighs=False):
        ...
        # all edges
        edge_index = [
            my_get_neighbors(x[i], self.num_points)
            for i in range(x.shape[0])
        ]
        neigh_idx = torch.stack(
            [edge_index[i] for i in range(x.shape[0])]
        )
        features_per_point = self.forward_per_point(x, start_neighs=neigh_idx) # dense_output_feature, B N F
        global_feature, _ = torch.max(features_per_point.transpose(1,2), 2)
        global_feature = global_feature.view(batch_size, -1)
...
import numpy as np
def my_get_neighbors(X, K):
    X = X.cpu().numpy()
    nbrs = NearestNeighbors(n_neighbors=K, algorithm='ball_tree').fit(X)
    distances, indices = nbrs.kneighbors(X)
    return torch.tensor(np.array(indices))